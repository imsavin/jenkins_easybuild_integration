        ...
        stage('Build') {
            steps {
                sh"""
                    ...
                    singularity run ${SINGULARITY_CONTAINER} $params.easyconfig --job --job-cores=16 --job-backend-config ./gc3pie.conf -r -l --job-output-dir=${EASYBUILD_PREFIX}/job_output/
                """
            }
        }
